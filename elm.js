...
var $author$project$Main$generateSample = function (size) {
	return A2(
		...
		A2(
			...
			size,
			A2($elm$random$Random$float, 10, 90)));
};
...
var $author$project$Main$showLog = function (speed) {
	return A2(
		...
		function (_v0) {
			return $author$project$Main$ShowLog;
		},
	...
	function (list, element, pivots, log) {
		...
		while (true) {
			if (!pivots.b) {
				...
				var end = _v1.c;
				var ps = pivots.b;
				if ((_Utils_cmp(element, end) > -1) && ((end - start) <= 1)) {
					if (!ps.b) {
						return log;
					} else {
						var _v3 = ps.a;
					...
					if (_Utils_cmp(element, end) > -1) {
						var newPivots = _Utils_ap(
							ps,
							_List_fromArray(
								[
									_Utils_Tuple3(start, start, pivot),
									_Utils_Tuple3(pivot + 1, pivot + 1, end)
								]));
						var _v4 = A2(
							$elm$core$Maybe$withDefault,
							_Utils_Tuple3(-1, -1, -1),
							$elm$core$List$head(newPivots));
						var nextPivot = _v4.a;
						var $temp$list = list,
							$temp$element = nextPivot + 1,
							$temp$pivots = newPivots,
							$temp$log = log;
						list = $temp$list;
						element = $temp$element;
						pivots = $temp$pivots;
						...
						var elId = _v6.a;
						var elValue = _v6.b;
						if (_Utils_cmp(elValue, pivValue) < 0) {
							var newList = _Utils_ap(
								A2($elm$core$List$take, pivot, list),
								A2(
									$elm$core$List$cons,
							...
							var $temp$list = newList,
								...
								$temp$pivots = A2(
								$elm$core$List$cons,
								_Utils_Tuple3(pivot + 1, start, end),
								ps),
								$temp$log = newLog;
							list = $temp$list;
							element = $temp$element;
...
var $author$project$Main$ChangeSorter = function (a) {
	return {$: 2, a: a};
};
...
var $author$project$Main$standardAttributes = function (_v0) {
	var index = _v0.a;
	var value = _v0.b;
	return _List_fromArray(
		[
...
var $author$project$Main$computeAttributes = F2(
	function (model, _v0) {
		var index = _v0.a;
		var value = _v0.b;
	...
	return {
		ay: _List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('sample-frame')
					]),
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('sample__wrapper')
							]),
						A2(
							$elm$core$List$map,
							function (sample) {
